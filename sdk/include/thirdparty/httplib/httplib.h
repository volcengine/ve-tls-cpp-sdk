...
public:
  ...
  Result CustomRequest(const std::string &method, const std::string &path, const Headers &headers,
                                  const Params &params, const std::string &body, const std::string &content_type);
...
public:
  ...
  Result CustomRequest(const std::string &method, const std::string path, const Headers &headers, const Params &params, const std::string &body, const std::string &content_type);
  Result Get(const std::string &path);
  Result Get(const std::string &path, const Headers &headers);
  Result Get(const std::string &path, Progress progress);
  Result Get(const std::string &path, const Headers &headers,
...
inline Result ClientImpl::CustomRequest(const std::string &method, const std::string &path, const Headers &headers,
                                        const Params &params, const std::string &body, const std::string &content_type) {
  std::string path_with_query = path;
  if (!params.empty()) {
    path_with_query = path + "?" + detail::params_to_query_str(params);
  }
  // printf("eren wants to know url: %s\n", path_with_query.c_str());
  // printf("eren wants to know body: %s\n", body.c_str());
  return send_with_content_provider(method, path_with_query, headers, body.data(), body.size(), nullptr, nullptr, content_type);
}
...
inline Result Client::CustomRequest(const std::string &method, const std::string path, const Headers &headers, const Params &params, const std::string &body, const std::string &content_type) {
  return cli_->CustomRequest(method, path, headers, params, body, content_type);
}
